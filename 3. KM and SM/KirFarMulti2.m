...
function KirFarMulti2(antenna,frequencylow,frequencyhigh)
...
Y=zeros(size(a,2),size(b,2),totalcount);

count=0;    
for frequency=frequencylow:0.01:frequencyhigh
    count=count+1;
    frequency_index=fix(1+(frequency-0.4)/0.01); % to fix the index as an integer
    A=eval(['wtum' num2str(antenna) '(:,:,frequency_index)']);
    ...
    for j=1:size(a,2)
        for l=1:size(b,2)
            ...
            for t=1:size(S,2)
                Y(l,j,count)=Y(l,j,count)+(S(t,t)^2*(TestVector'*U(:,t))*(TestVector'*conj(V(:,t))));
            end
        end
    end
end
...
% Save plotdata
eval(['save plotdataKirFarMulti2-' num2str(antenna) '.mat a b Y Z']);
