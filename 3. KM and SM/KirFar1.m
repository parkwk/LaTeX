...
function KirFar1(antenna,frequency)
...
for j=1:size(a,2)
    for l=1:size(b,2)
        ...
        Y(l,j)=TestVector'*(A-B)*conj(TestVector);
    end
end
% Boundary drawing
th=0:0.01:2*pi;
...
% Save plotdata
eval(['save plotdataKirFar1-' num2str(antenna) '-' num2str(frequency) '.mat a b c d Y']);

% Plotting
figure('position',[300,300,400,300]);
contourf(a,b,abs(Y)/max(max(abs(Y))));
hold on;
shading flat;
colorbar;
box on;
